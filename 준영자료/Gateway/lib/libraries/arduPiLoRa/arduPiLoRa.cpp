...
*/
boolean	SX1272::availableData(uint16_t wait)  //유효한 헤더가 기다리는 동안 기다리는 시간
{
	byte value;
	byte header = 0;
	boolean forme = false;
	boolean	_hreceived = false;
	unsigned long previous; unsigned long previous1;
	...
	{ // LoRa mode
		...
		//after = millis();
	//	std::cout <<"read IRQ1 regester : " <<(unsigned long)(millis()-previous) << std::endl;

	//	std::cout<<"time 1: "<< millis() - previous<<std::endl;


		while( (bitRead(value, 4) == 0) && (millis() - previous < (unsigned long)wait) )
		{ //bitRead 하는데 걸리는 시간 < wait 보다 짧으면 ?? read 할게없음 ? 
				//std::cout<<"wait Time: "<< (unsigned long)wait<<std::endl;
			//	std::cout<<"time 1: "<< millis() - previous<<std::endl;
		...
		//	 std::cout <<"read IRQ2 regester : " <<(unsigned long)(millis()-previous) << std::endl;
			/*
            ...
            //  std::cout <<"read FREG_IRQ_FLAGS regester : " <<(unsigned long)(millis()-previous) << std::endl;
		} // end while (millis)
		if( bitRead(value, 4) == 1 ) 
		{ // header received
			#if (SX1272_debug_mode > 0)
			...
			#endif
			_hreceived = true;
			while( (header == 0) && (millis() - previous < (unsigned long)wait) )  //wait
			{ // Waiting to read first payload bytes from packet
				//std::cout<<"wait Time: "<< (unsigned long)wait<<std::endl;
			//	std::cout<<"time 2: "<< millis() - previous<<std::endl;
			...
			/*
				if(header != 0)
				{
					std::cout <<"read FIFO RX BYTE ADDR regester : " <<(unsigned long)(millis()-previous1) << std::endl;
				}
              */ 
                 delay(1000);
				// Condition to avoid an overflow (DO NOT REMOVE)
            ...
            //    std::cout <<"read FIFO RX BYTE ADDR regester : " <<(unsigned long)(millis()-previous) << std::endl;
