...
string loraJSONString(MyPacket *my_packet){
	...
	string str_nictype;
		...
		root["tlength"] = my_packet->GetLoraPacketLen();
		...
		root["receivegeneratetime"] = static_cast<int> (my_packet->GetReceiveGenerateTime());
 		root["rssi"] = my_packet->GetLoraRssi();
		root["snr"] = my_packet->GetLoraSnr();
		root["packetrssi"] = my_packet->GetLoraPacketRssi();
