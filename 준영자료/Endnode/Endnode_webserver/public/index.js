...
function getClientTime(){
	...
	xhttp.open("GET", "http://192.168.102.1:4000", true);
	xhttp.open("GET", "http://192.168.102.1:4000/systemtime", true);
	xhttp.send();
}
...
for(var i=0;i<expIndex;i++) {
		...
		var sendTranceiverInfo = { src: null, dst: null, sqc: null };
			...
			if(Number(curExp.childNodes[6].childNodes[1].childNodes[1].value) < 10)
		 	...
		 	var url = "http://192.168.102.1:4000/exps"; //gateway
		    var xhttp = new XMLHttpRequest();
...
function stopExp() {
	var url = "http://192.168.102.1:4000/exps"; //gateway address
